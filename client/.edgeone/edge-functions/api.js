/* eslint-disable */
// ══════════════════════════════════════════════════════════════
// 快乐小孩 · EdgeOne Edge Function
// 处理所有 /api/* 请求（AI对话、人格分析、结局裁决、人格画像）
// ══════════════════════════════════════════════════════════════

// ── 环境变量通过 EdgeOne 控制台配置 ──
// MODEL_PROVIDER: "hunyuan" | "openai" | "mock"
// HUNYUAN_API_KEY / LLM_API_KEY / HUNYUAN_BASE_URL / LLM_BASE_URL ...
...
async function callOpenAI(env, prompt, mockType) {
  const apiKey = getEnv(env, "LLM_API_KEY");
  if (!apiKey) throw new Error("缺少 LLM_API_KEY");
  const baseUrl = (getEnv(env, "LLM_BASE_URL", "https://api.openai.com/v1")).replace(/\/+$/, "");
  const model = getEnv(env, "LLM_MODEL", "gpt-5.5");
  const reasoningEffort = getEnv(env, "LLM_REASONING_EFFORT", "low");
  const timeoutMs = Number(getEnv(env, "LLM_TIMEOUT_MS", "45000"));
  let lastError = null;
...
async function callHunyuan(env, prompt, mockType) {
  const apiKey = getEnv(env, "HUNYUAN_API_KEY");
  if (!apiKey) throw new Error("缺少 HUNYUAN_API_KEY");
  const baseUrl = (getEnv(env, "HUNYUAN_BASE_URL", "https://tokenhub.tencentmaas.com/v1")).replace(/\/+$/, "");
  const model = getEnv(env, "HUNYUAN_MODEL", "hunyuan-role-latest");
  const timeoutMs = Number(getEnv(env, "LLM_TIMEOUT_MS", "45000"));
  let lastError = null;
...
async function callLLM(env, prompt, mockType) {
  console.log("--- LLM PROMPT (first 500 chars) ---");
  console.log(prompt.slice(0, 500));

  const provider = getEnv(env, "MODEL_PROVIDER", "mock");
  if (provider === "openai") return callOpenAI(env, prompt, mockType);
  if (provider === "hunyuan") return callHunyuan(env, prompt, mockType);
...
// ═══════════════════════ PROMPT GENERATORS (inlined from server/src/prompts/*.ts) ═══════════════════════
...
function sceneFragmentPrompt(gameState, sceneId, mode, storyContext, scenePrompt, requiredLines, skeletonLines) {
  ...
  if (hasSkeleton && hasRequired) {
    structureSection = "═══════════════════════════════════\n" +
      ...
      "                          ↓\n" +
      "  ┌─ 阶段2：NPC追问（逐字出现必说台词）\n" +
      "  │ 紧接着阶段1的旁白结束之后，必须逐字出现「必说台词」区块中的NPC对话行。\n" +
      "  └─────────────────────────────────────\n" +
      "                          ↓\n" +
      "  ┌─ 阶段3：主角回应（自由创作，受动态分支约束）\n" +
      "  │ 根据「动态分支」规则生成主角的解释性回应。必须体现真诚而非说教。\n" +
      "  └─────────────────────────────────────\n" +
      "                          ↓\n" +
...
async function generatePersonalityPortrait(env, gameState) {
  ...
  let raw;
  try {
    raw = await callLLM(env, prompt, "personality_portrait");
  } catch {
    raw = { title: "凿孔者" };
  }
...
async function handleAnalyzeChoice(env, body) {
  const { gameState, choiceId } = body;
  const prompt = analyzeChoicePrompt(gameState, choiceId);
  const result = await callLLM(env, prompt, "choice_analysis");
  return { ok: true, result };
}

async function handleNpcDialogue(env, body) {
  const { gameState, npcId } = body;
  const prompt = npcDialoguePrompt(gameState, npcId);
  const result = await callLLM(env, prompt, "npc_dialogue");
  return { ok: true, result };
}

async function handleGenerateScene(env, body) {
  ...
  const normalizedRequiredLines = Array.isArray(requiredLines)
    ? requiredLines.filter(l => typeof l === "string") : [];
  const normalizedSkeletonLines = Array.isArray(skeletonLines)
    ? skeletonLines.filter(l => typeof l === "string") : [];
  ...
  const result = await callLLM(env, fullPrompt, "scene_fragment");
  ...
  if (typeof result.script === "string") {
    ...
    if (normalizedSkeletonLines.length > 0) {
      const skeletonMissing = normalizedSkeletonLines.filter(function(line) {
        const spoken = line.replace(/^\[[^\]]+\]\s*/, "");
        return spoken && result.script.indexOf(spoken) === -1;
      });
      if (skeletonMissing.length > 0) {
        console.warn("[AI] 骨架句缺失 scene=" + sceneId + " missing=" + skeletonMissing.length);
      }
    }
  }
  return { ok: true, result };
...
async function handleJudgeEnding(env, body) {
  const { gameState } = body;
  const prompt = endingJudgePrompt(gameState);
  const result = await callLLM(env, prompt, "ending_judge");
  return { ok: true, result };
}

async function handlePersonalityPortrait(env, body) {
  const { gameState } = body;
  const result = await generatePersonalityPortrait(env, gameState);
  return { ok: true, result };
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      switch (route) {
        case "analyze-choice":
          result = await handleAnalyzeChoice(env, body);
          break;
        case "npc-dialogue":
          result = await handleNpcDialogue(env, body);
          break;
        case "generate-scene":
          result = await handleGenerateScene(env, body);
          break;
        case "judge-ending":
          result = await handleJudgeEnding(env, body);
          break;
        case "personality-portrait":
          result = await handlePersonalityPortrait(env, body);
          break;
        default:
          return corsResponse({ ok: false, message: "unknown API endpoint: " + route }, 404);
      }

      return corsResponse(result, 200);
    } catch (error) {
      console.error("[Edge-Func] " + path + " failed:", error.message || String(error));
