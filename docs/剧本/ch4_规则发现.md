# 第4章 · 规则发现

## 一、说服自己
### [CG]探索进度提升
图片：
效果：淡入
[旁白]要是我没有看那段文字的话，是不是就可以避免“我”的负面思想？但是，如果我不选择冒险探寻“我”内心真实的想法，又怎么让我得到真正的快乐呢？
[NPC:系统]参赛者寻找到“被遗弃的呐喊碎片1”，副本探索度达5%。
[旁白]为了拿到足够通关奖励，我需要努力把探索度加到100%。每天解锁一片碎片，一周可以增加35%的探索度。\n\n要安全收集碎片，必须每天创造让“我”快乐的回忆。
[旁白]我合上计划本，打开手机通讯录，给刘宇发了条信息，就上床睡觉了。\n\n当然，我这一夜睡得并不好。
[旁白]第二天，我早早来到学校，在教室所在楼层绕了一圈，没有发现任何规则。\n\n我回到教室，在多出来的空位上坐下。
→ 跳转：ch4_find_brochure
### [地图]寻找宣传册（探索）
地图：classroom
效果：淡入
出生点：spawn_classroom_default
玩家状态：sit_up
冻结主角：是
[旁白]过了一分钟，周围的同学没有注意到我，我身上也没有发生任何怪异的事。\n\n确认不会违规后，我开始调查抽屉。除了一堆作业之外，我还意外发现了一张活动宣传手册。
### [CG]教室规则
图片：
效果：淡入
[旁白]手册里夹着一张纸，我取出那张纸，看到了熟悉的猩红字迹。
[旁白]1.严格遵照课程表安排的课程上课，上课须专心，禁止交头接耳（讨论和回答问题除外），禁止离开座位，禁止进食。\n\n2.课堂问题回答错误会受到惩罚。\n\n3.午餐和晚餐由学校统一提供，禁止浪费。\n\n4.每日晚自习进行周测，阅卷完毕后由各科老师公布成绩和排名，请学生们努力学习。\n\n5.各班单科周测成绩排名后五名将进行为期一周的课外辅导，请在晚自习开始前30分钟准时到达各班教室参加辅导。\n\n6.当天作业通过学习平板提交，请注意作业截止时间，禁止缺交。\n\n7.晚自习禁止进食、交头接耳、离开座位。非必要不要抬头。\n\n8.学生要听老师的话，禁止忤逆老师。\n\n9.晚自习中有30分钟的答疑时间，各科老师会在门口待命，答疑时间及科目每天由各班班长在后黑板更新，请有疑问的同学积极提问。
[主角]（这个规则并不公平。）
[旁白]我们班是尖子班，第五条规则显然是不合理的。不过，这也方便我通过第五条规则获得额外信息。\n\n还有第九条规则，并没有说明向老师提问的内容限制，如果可以从老师口中套出情报的话，一定要抓住这个机会。\n\n而第三条规则让我感觉是一个陷阱。我目前还不知道食用副本中的食物会有什么副作用，这条规则强制我食用校内食物，无论遵守还是不遵守都是死路一条。
[主角]（我需要找到一个合理处理食物的方法。）
[旁白]收起规则，我转而翻开那张活动宣传册。
### [CG]宣传册内容
图片：
效果：淡入
[旁白]这是一个校园试胆活动的宣传册，活动时间是本周六晚19：00，也就是即将通过副本的那一天。扫二维码即可报名，限制名额五人。
[主角]（这活动我是肯定要报名的。）
[旁白]此次试胆活动的主题是调查在学校内流传已久的怪谈，具体内容将在报名后告知。
### [地图]早上教室
地图：classroom
效果：淡入
出生点：spawn_classroom_default
玩家状态：sit_up
NPC：
  - id: npc_liuyu, 名称: 刘宇, 精灵: ly_frames, 位置: spawn_liuyu_seat, 朝向: 下
冻结主角：是
[旁白]我拿走宣传册，回到自己的座位上，开始按计划表学习。
[旁白]十五分钟后，刘宇来了，他径直走到我面前，把一张花名册扔到我桌上。
NPC动作：
  npc_liuyu → 播放动画: walk_down → 移动至: spawn_player_desk → 面朝: 主角
[NPC:刘宇]哝，你要的东西。
[主角说]（笑）谢谢，帮大忙了。
[NPC:刘宇]没事。我先收作业去了。
NPC动作：
  npc_liuyu → 播放动画: walk_up → 移动至: spawn_liuyu_seat → 播放动画: sit_up
[主角]（作为班长，刘宇要遵守的规则是不是更多？但同样地，他知道的情报会比其他学生更多，甚至还掌握部分老师的信息。）
[旁白]我暗做打算，然后端详起花名册。
### [CG]花名册

---

## 二、看不见的名字

> 导演说明：
> - 本章固定事实：花名册中存在一个主角无法读取、无法听清姓名的学生。
> - AI动态部分：刘宇与普通同学如何评价主角的调查方式，以及刘宇是否提前把主角视为“值得合作的人”。
> - 无论玩家如何选择，都不能提前揭露透明学生身份。

### [CG]花名册异常

图片：花名册特写（待补）
效果：轻微失焦
[旁白]花名册上共有三十九人。大部分名字都很熟悉，唯独多出来的那个人——
[主角]（……？）
[旁白]那明明是三个端正的汉字，我却无法理解它们。每当视线试图聚焦，笔画便像水中的倒影一样散开。

→ 选项：直接询问附近同学，这三个字怎么读
    AI标签：主动验证, 直接, 承担暴露风险
    NPC目睹：刘宇
    设置flag: ch4_roster_direct
    跳转：ch4_roster_ask_student

→ 选项：先对照座位与花名册，确认这个人是否真实存在
    AI标签：谨慎调查, 现实判断, 自我保护
    NPC目睹：刘宇
    设置flag: ch4_roster_observe
    跳转：ch4_roster_observe

→ 选项：把花名册推给刘宇，用眼神示意异常的位置
    AI标签：试探合作, 信任, 保留情报
    NPC目睹：刘宇
    设置flag: ch4_roster_test_liuyu
    跳转：ch4_roster_test_liuyu

### [地图]直接询问同学

地图：classroom
出生点：spawn_classroom_default
冻结玩家：是
NPC动作：
  npc_classmate → 面朝: 主角
[主角说]同学，你知道这三个字是什么吗？
[NPC:同学]你是不是傻啊？这不是我们班的——
→ 音效: tinnitus (once)
→ 效果: flash_screen
[旁白]她念出名字的瞬间，尖锐的耳鸣刺穿了我的脑袋。声音明明近在咫尺，我却一个字也没能听清。
[主角]（看来我无法通过这条渠道获取透明人的信息。）
→ NPC观念更新: 刘宇
→ 跳转：ch4_roster_converge

### [地图]对照座位调查

地图：classroom
出生点：spawn_classroom_default
冻结玩家：是
[旁白]我没有立刻开口，而是借着整理作业的动作，逐一核对座位与花名册。
[旁白]三十九个名字，三十八张正常使用的桌椅，以及一张所有人都会自然绕开的空座位。
[主角]（不是没人看见它。他们只是不认为那里“空着”。）
→ 音效: tinnitus (once)
[旁白]我再次试图默念那三个字，耳鸣立刻从脑海深处响起，像某种温和却坚决的警告。
[主角]（这个名字本身受到规则保护。）
→ NPC观念更新: 刘宇
→ 跳转：ch4_roster_converge

### [AI对话]用花名册试探刘宇

背景：早晨教室，刘宇正在收作业，其他学生在场
NPC自主扮演: 刘宇
AI提示：主角没有公开提问，而是把花名册异常位置推给刘宇看。
        固定事实：刘宇知道主角无法读取那个名字，但不能说出名字、身份或原因。
        根据刘宇对主角当前印象回应：
        - 若主角此前表现谨慎且愿意合作，刘宇用玩笑掩饰，提醒“有些东西看不清就别硬看”，并认可主角的默契。
        - 若主角此前拒绝合作或表现鲁莽，刘宇装作没看懂，只用眼神警告主角别在教室公开调查。
        - 不得提供学校根本规则，不得承认自己受系统控制。
→ NPC观念更新: 刘宇
→ 对话结束后：跳转 ch4_roster_converge

### [CG]花名册结论

[旁白]无论通过视觉、语言还是他人的反应，我都无法得到那个名字。
[主角]（这不是普通的信息缺失，而是一条正在主动阻止我理解某个人的规则。）
[旁白]我把花名册收进抽屉。现在还不是继续追查的时候。
→ 设置flag: ch4_roster_anomaly_found
→ 跳转：ch4_class_montage

---

## 三、对玩家有利的惩罚

### [CG]前三节课

[旁白]前三节课风平浪静地过去了。
[旁白]没有学生主动回答问题。老师不断重复讲题、做题、对答案，再讲题。所有人都在认真忍受，像一群被卡在同一分钟里的钟表。
[主角]（痛苦不会让时间停止。只要继续坐着，它总会过去。）
→ 跳转：ch4_physics_observe

### [地图]物理课观察

地图：classroom
出生点：spawn_classroom_default
冻结玩家：是
NPC：
  - id: npc_teacher_li, 名称: 李老师, 精灵: teacher_frames, 位置: spawn_teacher_front, 朝向: 下
[旁白]第四节是物理课。李老师严格、古板，尤其擅长用带有人身攻击性的批评摧毁学生的自信。
[旁白]反常的是，在他的威压之下，反而有不少学生主动举手，然后故意回答错误。
NPC动作：
  npc_teacher_li → 播放动画: idle_down
[NPC:李老师]这么简单的问题都不会？你们到底有没有认真听课？
[旁白]被骂的学生低下头，神情却悄悄放松下来。没有举手的人坐得笔直，双眼空洞地盯着讲台。
[主角]（这个班没有傻子。不惜被骂也要获得的东西，是惩罚。）

→ 选项：主动举手，故意回答一个错误答案
    AI标签：规则利用, 主动冒险, 现实判断, 承担羞辱
    NPC目睹：周骐瑞, 刘宇
    设置flag: ch4_lunch_punished
    跳转：ch4_physics_wrong_answer

→ 选项：暂时不行动，先确认惩罚内容
    AI标签：谨慎观察, 自我保护, 延迟行动
    NPC目睹：周骐瑞, 刘宇
    设置flag: ch4_lunch_not_punished
    跳转：ch4_physics_hold_back

### [地图]故意答错

地图：classroom
冻结玩家：是
NPC动作：
  npc_teacher_li → 面朝: 主角
[主角说]老师，我认为答案是……
[旁白]我给出了一个听起来经过思考、实际上错得恰到好处的答案。
[NPC:李老师]错得离谱！下课后自己好好反省！
[旁白]他骂完便继续讲课，熟练得像完成了一项每日任务。
[主角]（希望这份惩罚值得我挨这顿骂。）
→ 跳转：ch4_lunch_punishment_reveal

### [地图]保持观察

地图：classroom
冻结玩家：是
[旁白]我没有举手。未知的惩罚可能是出口，也可能是另一层陷阱。
[旁白]周骐瑞在李老师抛出问题时主动举手，平静地给出了错误答案。被骂之后，他甚至没有皱一下眉。
[主角]（他知道惩罚内容。午饭时跟着他，应该能找到答案。）
→ 跳转：ch4_lunch_punishment_reveal

### [CG]午饭惩罚公布

[NPC:物理课代表]名单上的同学课上回答问题错误，受到的惩罚是——不准吃午饭。
[主角]（我的天，这个惩罚好啊。）

条件：ch4_lunch_punished
[旁白]我没忍住咧嘴笑了起来。周围几名同学用一种难以理解的眼神看着我。
→ 设置flag: ch4_food_escape_confirmed
→ 跳转：ch4_zhou_lunch_approach

条件：ch4_lunch_not_punished
[旁白]答案已经确认，但我没有获得免除午餐的资格。现在我必须在“不许浪费”的规则下，找到另一条处理食物的路。
→ 跳转：ch4_zhou_lunch_approach

---

## 四、周骐瑞的是非问答

> 导演说明：
> - 周骐瑞是“行动型知情者”，不会主动照顾主角，也厌恶没有边界的追问。
> - 固定最低线索：校内食物会让人失去自我；除惩罚外存在其他处理方法；周骐瑞会参加周六活动。
> - AI只调整他的态度、回答的完整度，以及是否额外提醒晚餐处理时机。

### [地图]午休寻找周骐瑞

地图：classroom
出生点：spawn_classroom_default
冻结玩家：否
NPC：
  - id: npc_zqr, 名称: 周骐瑞, 精灵: zqr_frames, 位置: spawn_zqr_seat, 朝向: 下

@trigger_zqr_lunch
  提示文字：E询问周骐瑞
  触发：[对话]
    [NPC:周骐瑞]怎么，有事找我？
    → 跳转：ch4_zhou_question_method

### [地图]选择提问方式

[主角]（他可能无法直接回答。问题怎么问，决定了他是否愿意继续理我。）

→ 选项：提前说明限制，只要求他用“是”或“否”回答
    AI标签：尊重边界, 逻辑推理, 谨慎合作
    NPC目睹：周骐瑞
    设置flag: ch4_zhou_respectful
    跳转：ch4_zhou_fixed_questions

→ 选项：坦白自己可能会死，请他直接告诉我处理方法
    AI标签：坦诚求助, 信任, 暴露脆弱
    NPC目睹：周骐瑞
    设置flag: ch4_zhou_frank
    跳转：ch4_zhou_fixed_questions

→ 选项：先说出自己的推测，观察他的表情验证答案
    AI标签：试探, 自我保护, 控制信息
    NPC目睹：周骐瑞
    设置flag: ch4_zhou_testing
    跳转：ch4_zhou_fixed_questions

### [CG]周骐瑞固定问答

[主角说]这里的食物会让人失去自我意识，对吗？
[NPC:周骐瑞]是。
[主角说]除了通过惩罚免除午晚餐，还有其他处理方法？
[NPC:周骐瑞]是。
[主角说]你打算参加这周六的试胆活动？
[NPC:周骐瑞]是。
[主角]（看来同学们都知道这个活动。）
→ 跳转：ch4_zhou_dynamic_response

### [AI对话]周骐瑞判断主角

背景：午休教室，周围学生安静写作业
NPC自主扮演: 周骐瑞
AI提示：根据玩家本章的调查方式、物理课选择与提问方式，判断周骐瑞对主角的印象。
        固定事实不能改变：学校食物危险；存在其他处理方法；周骐瑞参加周六活动。
        动态回应范围：
        - 高现实判断/尊重边界：认可主角会利用规则，可额外提醒“晚餐动作要快，去晚了就没位置处理”。
        - 高坦诚信任：不表现亲近，但可明确建议主角去找刘宇，认为刘宇会处理。
        - 高试探/低信任：只说“你已经得到答案了”，不提供额外帮助。
        - 高鲁莽/低现实判断：警告主角不要把所有漏洞都当作安全出口。
        不得解释学校根本规则，不得透露周六活动具体内容。
→ NPC观念更新: 周骐瑞
→ 对话结束后：跳转 ch4_art_class_start

---

## 五、美术课：自由之后画什么

> 导演说明：
> - 王老师必须说出“做你自己”这一核心命题，但他对主角“虚伪”的判断不应机械固定。
> - AI根据玩家画作与既往人格画像生成评价；评价可以不同，但不能直接给出快乐或通关答案。
> - 王老师对主角的印象会影响第5章交易：欣赏真诚者、警惕表演者、刺激服从者。

### [地图]美术课开始

地图：classroom
出生点：spawn_classroom_default
冻结玩家：是
NPC：
  - id: npc_wang_teacher, 名称: 王沁林, 精灵: teacher_frames, 位置: spawn_teacher_front, 朝向: 下
NPC动作：
  npc_wang_teacher → 播放动画: walk_down → 移动至: spawn_teacher_front → 面朝: 下
[旁白]周一下午有一节美术课。一位将近七十岁的老人步伐稳健地走进教室，澄澈锐利的眼睛扫过每个人的脸。
[NPC:王沁林]把桌上的书都收起来吧，什么都别放。
[旁白]窸窸窣窣的收拾声响起。同学们等待着新的标准答案。
[NPC:王沁林]这节课完成一幅画。画什么都可以，不必完成，我也不会夸奖或批评任何人。
[NPC:王沁林]在我的课上没有竞争。你们只要成为你们自己就好。
[旁白]短暂的自由落下来，教室里却没有一个人动笔。
[主角]（当一个人只学会了服从，连自由也会变成一道不会做的题。）
→ 跳转：ch4_choose_painting

### [地图]选择画作

→ 选项：画四分五裂的傀儡与监牢，用隐喻试探王老师
    AI标签：策略表达, 真相欲望, 表演性反抗
    NPC目睹：王沁林, 刘宇, 周骐瑞
    设置flag: ch4_painting_puppet
    跳转：ch4_show_painting

→ 选项：画一段真正令自己放松的普通回忆，不考虑它是否“深刻”
    AI标签：真诚表达, 快乐感知, 暴露自我
    NPC目睹：王沁林, 刘宇, 周骐瑞
    设置flag: ch4_painting_memory
    跳转：ch4_show_painting

→ 选项：画一幅标准优秀学生作品，先观察王老师是否真允许自由
    AI标签：谨慎服从, 自我保护, 怀疑权威
    NPC目睹：王沁林, 刘宇, 周骐瑞
    设置flag: ch4_painting_safe
    跳转：ch4_show_painting

### [地图]展示画作

地图：classroom
冻结玩家：是
NPC动作：
  npc_wang_teacher → 播放动画: walk_down → 移动至: spawn_player_desk → 面朝: 主角
[主角说]老师，我画完了。
[旁白]附近的同学惊恐地盯着我，仿佛第一个在自由里行动的人，比违反规则的人更加危险。
→ 跳转：ch4_wang_dynamic_judgment

### [AI对话]王老师评价画作

背景：美术课教室，王老师站在主角桌旁，全班能够听见部分对话
NPC自主扮演: 王沁林
AI提示：王老师评价玩家选择的画作，并结合七维人格画像判断主角是在真诚表达、策略试探，还是用服从保护自己。
        必须遵守：
        - 王老师欣赏矛盾与主体性，但不会无条件赞扬。
        - 必须自然引出“做你自己”这一命题。
        - 若画傀儡监牢：指出画面“混乱中有决绝”，同时指出它可能是为了套取答案而画，因此“和现在的你一样虚伪”。
        - 若画真实回忆：承认画面普通却真实，追问主角为何认为快乐必须足够深刻才值得被画。
        - 若画标准作品：指出技巧正确但画里没有人，询问主角是在观察老师，还是害怕自由。
        - 不能直接解释副本真相，不能直接给出快乐答案，不能替玩家定义真实自我。
→ NPC观念更新: 王沁林
→ 对话结束后：跳转 ch4_wang_question_choice

### [地图]向王老师追问

[主角]（他看穿了我的目的，但没有赶我走。这可能是一次机会。）

→ 选项：用画作继续打哑谜：人偶怎样才能坐到监牢外的椅子上？
    AI标签：隐喻追问, 真相欲望, 保持距离
    NPC目睹：王沁林
    设置flag: ch4_wang_metaphor
    跳转：ch4_wang_core_reply

→ 选项：承认自己害怕被规则同化，询问“做自己”是否意味着送死
    AI标签：坦诚, 自我认知, 承担脆弱
    NPC目睹：王沁林
    设置flag: ch4_wang_honest
    跳转：ch4_wang_core_reply

→ 选项：暂不追问，把他的评价记下来
    AI标签：克制, 自我保护, 延迟判断
    NPC目睹：王沁林
    设置flag: ch4_wang_withhold
    跳转：ch4_wang_core_reply

### [CG]王老师的核心回应

条件：ch4_wang_metaphor
[NPC:王沁林]你早就有能力坐到那张椅子上了，不是吗？
[主角说]但我不知道该怎么办。这样的话，哪怕有能力也没有用。
[NPC:王沁林]所以啊，我一开始就已经说了——做你自己就好。

条件：ch4_wang_honest
[NPC:王沁林]做自己不等于把命丢掉。若一个人只在安全时才承认自己，那份“自己”也未免太轻了。
[NPC:王沁林]至于什么时候该冒险，那是你的课题，不是我的答案。

条件：ch4_wang_withhold
[NPC:王沁林]能忍住提问也是一种选择。但别把等待答案，误当成正在寻找答案。

[旁白]他的目光重新投向整个教室。
[NPC:王沁林]这是你们从生到死都应该放在首位的生命课题，孩子们。
→ 设置flag: ch4_wang_noticed_player
→ 跳转：ch4_dinner_problem

---

## 六、晚餐：求助也是一种行动

### [CG]盒饭难题

[旁白]下午没有物理课，真是可惜。
[旁白]我看着桌上的盒饭，比面对一道数学压轴题还要焦头烂额。
[主角]（NPC们似乎知道我是参赛者，只不过他们愿意陪我演戏。）
[旁白]刘宇拿着盒饭从我身边经过。

→ 选项：揪住刘宇衣角，直接请他带自己处理盒饭
    AI标签：主动求助, 信任, 放下面子
    NPC目睹：刘宇
    设置flag: ch4_asked_liuyu_directly
    跳转：ch4_liuyu_food_response

→ 选项：提出自己的推测，请刘宇确认厕所是否可用于处理食物
    AI标签：合作提案, 现实判断, 保留主动权
    NPC目睹：刘宇
    设置flag: ch4_asked_liuyu_plan
    跳转：ch4_liuyu_food_response

→ 选项：不向刘宇开口，观察他离开的方向并自行跟上
    AI标签：独立行动, 怀疑, 自我保护
    NPC目睹：刘宇
    设置flag: ch4_followed_liuyu
    跳转：ch4_liuyu_food_response

### [AI对话]刘宇决定帮到什么程度

背景：晚餐时间的教室门口，学生陆续领取盒饭
NPC自主扮演: 刘宇
AI提示：刘宇判断主角是否值得进一步合作。
        固定主线：主角最终会得知厕所可以处理盒饭，并安全避开晚餐。
        动态帮助等级：
        - 高信任且主动求助：刘宇嘴上嫌弃，实际带路并提醒动作要快。
        - 高现实判断且提出方案：刘宇确认方案，允许主角同行，并认可其推理。
        - 低信任或尾随：刘宇当场识破，要求主角自己说明目的；若主角此前表现不鲁莽，仍给出方向，但不主动照看。
        - 若玩家此前尊重周骐瑞边界或真诚面对王老师，可让刘宇注意到这种变化，但不能直接读取玩家内心。
        不得让刘宇拒绝到导致主线中断，不得泄露学校根本规则。
→ NPC观念更新: 刘宇
→ 对话结束后：跳转 ch4_dispose_food

### [CG]厕所处理盒饭

[旁白]厕所里已经挤了不少人。没有人交谈，大家默契地把饭倒进隔间，再用水冲掉。
[主角说]就把饭倒里头就完事了？
[NPC:刘宇]不然你以为还能怎么样？
[主角说]这么简单粗暴，真是出乎预料。
[NPC:刘宇]这是没有办法的办法。幸好来得早，不然待会连给你倒饭的位置都没有。
[旁白]规则要求“不许浪费”，却没有规定食物最后必须进入谁的胃里。
[主角]（所谓规则漏洞，往往不是规则没有想到，而是有人在缝隙里替后来者留了一条路。）
→ 设置flag: ch4_school_food_solution_found
→ 跳转：ch4_greenbelt_start

---

## 七、绿化带：合作已经开始

> 导演说明：
> - 本场景作为第4章的动态收束，也是第5章正式谈判的入口。
> - 刘宇必须传达三项固定信息：不要私自调查其他教室和办公室；教室规则不等于学校规则；学校规则无法靠普通搜索找到。
> - AI根据刘宇印象决定额外线索、语气和合作条件，不改变固定信息。

### [地图]前往绿化带

地图：gate
出生点：spawn_gate_default
冻结玩家：是
NPC：
  - id: npc_liuyu, 名称: 刘宇, 精灵: ly_frames, 位置: spawn_liuyu_greenbelt, 朝向: 左
[旁白]现在是十八点，离晚自习还有一个小时。
[旁白]刘宇像是看出了我还有问题，转身往教学楼外的绿化带走去。
NPC动作：
  npc_liuyu → 播放动画: walk_left → 移动至: spawn_liuyu_greenbelt → 面朝: 主角
[NPC:刘宇]来吧，到那里说。
[旁白]灌木隔开了来往视线。我们在两张石凳上坐下，终于获得了一小块可以说话的空间。
→ 跳转：ch4_liuyu_fixed_warning

### [CG]刘宇的固定警告

[NPC:刘宇]你家的事还牵扯到了学校里？
[主角说]多少有一些。
[NPC:刘宇]那我劝你，不要私自调查我们教室之外的其他教室和办公室。
[主角说]这不太合理吧？这么大的学校，不探索难道坐以待毙？
[NPC:刘宇]你今天看到的是教室里的规则，不是学校里的规则。
[主角说]所以我才更应该找到学校规则。
[NPC:刘宇]太具体的我不能说。但我可以告诉你——你找不到学校的规则。
[主角]（要知晓规则，就必须向NPC求助，然后受制于人。）
→ 跳转：ch4_liuyu_cooperation_choice

### [地图]回应刘宇

→ 选项：承认需要合作，但要求交换信息并划清边界
    AI标签：理性合作, 边界感, 现实判断
    NPC目睹：刘宇
    设置flag: ch4_liuyu_boundary_cooperation
    跳转：ch4_liuyu_dynamic_conclusion

→ 选项：坦白自己现在确实需要他的帮助，并询问合作条件
    AI标签：坦诚信任, 主动合作, 暴露需求
    NPC目睹：刘宇
    设置flag: ch4_liuyu_trusting_cooperation
    跳转：ch4_liuyu_dynamic_conclusion

→ 选项：指出刘宇一直在筛选自己，要求他先证明诚意
    AI标签：质疑, 谈判, 自我保护
    NPC目睹：刘宇
    设置flag: ch4_liuyu_demand_proof
    跳转：ch4_liuyu_dynamic_conclusion

→ 选项：拒绝承诺合作，只接受本次警告
    AI标签：独立, 高警惕, 拒绝关系约束
    NPC目睹：刘宇
    设置flag: ch4_liuyu_refuse_commitment
    跳转：ch4_liuyu_dynamic_conclusion

### [AI对话]刘宇确认合作关系

背景：傍晚绿化带，晚自习前，只有主角与刘宇
NPC自主扮演: 刘宇
AI提示：这是第4章的关系结算节点。综合玩家本章行为与刘宇此前印象，生成一段试探性回应。
        固定事实：
        - 刘宇认为双方事实上已经开始合作。
        - 刘宇不能直接说出学校根本规则。
        - 刘宇不会在此处交出完整地图或全部情报，那属于第5章正式谈判。
        动态结果：
        - 高信任+高现实判断：刘宇承认主角值得合作，并暗示晚自习答疑时间可能是安全提问窗口。
        - 高真相欲+高鲁莽：刘宇愿意合作，但要求主角行动前先通知他，否则不会救场。
        - 高自保+低信任：刘宇不强求口头承诺，只指出主角已经接受了他的多次帮助，并要求下一次用情报交换。
        - 高共情或真诚：刘宇减少玩笑，短暂表现真实关心。
        - 低信任且拒绝合作：刘宇仍保留联系渠道，但第5章初始帮助等级降低。
        输出必须自然落到“我们不是早就已经合作了吗？”这一含义，可调整具体措辞。
→ NPC观念更新: 刘宇
→ 对话结束后：跳转 ch4_cooperation_end

### [CG]章节收束

[旁白]我忽然意识到，从向刘宇索要花名册、接受食物处理方法，到坐在这里听他的警告，合作早已发生。
[主角]（问题从来不是要不要合作，而是我要以什么方式与他合作。）
→ 设置flag: ch4_liuyu_cooperation_opened
→ 跳转：ch5_liuyu_negotiate
