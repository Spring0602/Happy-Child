...
flowchart TD
    ...
    B --> H[Cloudflare Pages Functions]
    H --> I[/api/* AI API]
    I --> J[AI Prompt 模块]
    J --> K[LLM 大模型服务]
    K --> I
...
后端部署在 Cloudflare Pages Functions 的 `/api/*` 路径。支持三种模式，由环境变量 `MODEL_PROVIDER` 控制：`openai`（OpenAI API）、`hunyuan`（腾讯混元）、`mock`（离线兜底）。AI 生成失败时自动回退到 hand-written fallback 文本。

如果模型服务要求固定出口 IP，可以把 `server/` 部署到一台有固定公网 IP 的机器上作为 AI 中转服务，然后在 Cloudflare Pages 生产环境设置：

```env
AI_PROXY_URL=https://your-fixed-ip-api.example.com
AI_PROXY_TOKEN=一段足够长的随机密钥
```

中转服务机器上的 `server/.env` 设置同一个 `AI_PROXY_TOKEN`，并设置 `MODEL_PROVIDER=hunyuan`、`HUNYUAN_API_KEY` 等真实模型变量。此时前端仍请求 `/api/*`，Cloudflare Function 只负责转发，真正调用混元的是固定 IP 中转机；混元白名单填写这台中转机的公网出口 IP。

本地 `npm run dev` 默认通过 `client/.env.development` 使用线上 Cloudflare Pages API：`https://happy-child-game.pages.dev/api/*`。这适合“前端本地、AI 后端公网”的调试方式。

如果需要调试本机 Pages Functions，不要直接从 Vite 端口进入游戏，否则 `/api/*` 请求会打到 Vite dev server，前端会显示 AI 服务连接失败。推荐在 `client` 目录使用两个终端：
...
然后打开 `http://localhost:8788`。本地模型环境变量放在 `client/.dev.vars`，可从 `client/.dev.vars.example` 复制。
