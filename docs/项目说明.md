# 《快乐小孩》AI 叙事游戏项目说明

本文档用于给编程 AI、团队成员或后续开发者快速理解项目结构。

## 1. 项目目标

制作一个可在浏览器中运行的 AI 叙事视觉小说原型。

核心体验：

> 玩家在副本《快乐小孩》中做出选择，系统记录选择历史并形成玩家人格画像；AI 根据玩家画像动态调整 NPC 线索释放与结局裁决，使结局不再是固定路线奖励，而是玩家长期选择自然导向的命运结果。

## 2. 技术栈

```mermaid
flowchart TD
    A[浏览器玩家] --> B[Client 前端]
    B --> C[React + TypeScript]
    C --> D[Phaser 像素地图探索]
    C --> E[DialogOverlay 对话叠层]
    C --> F[剧情状态机]
    C --> G[本地存档 localStorage]
    B --> H[Server 后端]
    H --> I[Express API]
    I --> J[AI Prompt 模块]
    J --> K[LLM 大模型服务]
    K --> I
    I --> B
```

当前后端默认是 `mock AI`，不会真正调用大模型。后续只需要替换 `server/src/services/llm.ts`。

## 3. 总体玩法流程

```mermaid
flowchart TD
    Start[开始游戏] --> S1[显示剧情节点 Scene]
    S1 --> HasChoice{是否有玩家选项?}
    HasChoice -->|是| Choice[玩家点击选项]
    Choice --> Apply[applyChoice 更新人格值]
    Apply --> Save[保存 GameState]
    Save --> NeedAI{是否需要 AI 分析?}
    NeedAI -->|是| AIChoice[请求 /api/analyze-choice]
    AIChoice --> Trace[保存 AI 分析结果]
    NeedAI -->|否| NextScene[进入下一剧情节点]
    Trace --> NextScene
    HasChoice -->|否| HasNext{是否有 nextSceneId?}
    HasNext -->|是| NextButton[点击继续]
    NextButton --> NextScene
    HasNext -->|否| AIEvent{是否为 AI 事件?}
    AIEvent -->|NPC对话| NPCAPI[请求 /api/npc-dialogue]
    AIEvent -->|结局裁决| EndingAPI[请求 /api/judge-ending]
    NPCAPI --> Trace
    EndingAPI --> Trace
    NextScene --> S1
```

## 4. 前端状态结构

核心状态在 `client/src/types/game.ts` 的 `GameState` 中定义。

```mermaid
classDiagram
    class GameState {
        string currentSceneId
        Traits traits
        string[] choiceHistory
        Record npcTrust
        number exploration
        number rebellion
        number joyProof
        AITrace[] aiTraces
        string currentMapId
        PlayerPosition playerPosition
        Record flags
        string[] interactedItems
    }

    class Traits {
        number authorityResistance
        number truthDesire
        number selfProtection
        number empathy
        number realityJudgment
        number trust
        number joyPerception
    }

    class Scene {
        string id
        string chapter
        string speaker
        string character
        string text
        Choice[] choices
        string nextSceneId
        string aiEvent
    }

    class Choice {
        string id
        string text
        string nextSceneId
        effects
        tags
        boolean needAIAnalysis
    }

    GameState --> Traits
    Scene --> Choice
```

> **架构变更（2026-05）**：游戏不再采用 explore/narrative 双模式切换架构。改为**融合架构**——Phaser 像素地图始终可见，剧情触发时以 DialogOverlay 浮动叠层显示对话。因此 GameState 中不再有 `mode` 字段，改为通过 `DIALOG_START`/`DIALOG_END` 控制对话框的显示/隐藏。Scene 中 `background` 字段已弃用（场景由 Phaser 实时渲染）。

## 5. 文件职责总览

```mermaid
flowchart LR
    subgraph Client[client 前端]
        App[App.tsx 游戏入口]
        Dialog[DialogOverlay.tsx 对话叠层]
        Phaser[PhaserContainer.tsx Phaser引擎容器]
        Data[data/scenes.ts 剧情数据]
        Engine[engine/gameReducer.ts 状态机]
        Apply[engine/applyChoice.ts 选项影响]
        Save[engine/save.ts 本地存档]
        AIClient[services/aiClient.ts 请求后端]
        Types[types/game.ts 类型定义]
        GameDir[game/ 目录 Phaser场景+系统+Bridge]
    end

    subgraph Server[server 后端]
        Index[index.ts 后端入口]
        Routes[routes/ai.ts AI接口]
        Prompts[prompts/*.ts 提示词]
        LLM[services/llm.ts 模型调用]
    end

    App --> Dialog
    App --> Phaser
    App --> Engine
    App --> Save
    App --> AIClient
    Dialog --> Data
    Phaser --> GameDir
    Engine --> Apply
    AIClient --> Routes
    Routes --> Prompts
    Routes --> LLM
```

## 6. 关键模块说明

### 6.1 剧情节点 `data/scenes.ts`

每个 `Scene` 是一个视觉小说节点。

最常见三类节点：

1. 普通剧情节点：有 `nextSceneId`，点击“继续”进入下一段。
2. 选择节点：有 `choices`，点击选项后修改人格值并跳转。
3. AI事件节点：有 `aiEvent`，点击“启动 AI 分析”请求后端。

添加新剧情时，只需要新增一个 scene，然后让前一个节点的 `nextSceneId` 或某个 choice 指向它。

### 6.2 人格画像 `traits`

当前有七个维度，对应**认知→行动→影响**三层评判框架：

| 层次 | 字段 | 中文名 | 用途 |
|---|---|---|---|
| **认知层** | `truthDesire` | 真相欲望 | 是否愿意追问副本本质；衡量玩家是否看见了真实自我 |
| **认知层** | `joyPerception` | 快乐感知 | 是否能从功利评价之外感受生命；认同"我"只是第一步 |
| **行动层** | `authorityResistance` | 权威抵制度 | 是否质疑父母、老师、学校规则；区分"无脑叛逆"与"建设性叛逆" |
| **行动层** | `realityJudgment` | 现实判断 | 是否知道什么时候退让、什么时候冒险；防止高反抗但低策略导致自毁 |
| **行动层** | `selfProtection` | 自我保护 | 是否保留筹码、避免暴露；但是否因过度保护而无所作为 |
| **影响层** | `empathy` | 共情能力 | 是否理解 NPC 与"我"的痛苦；是否尝试影响周围的人 |
| **影响层** | `trust` | 关系信任 | 是否愿意与 NPC 合作；真正的进化是打破丛林法则，为世界带来光 |

**设计理念**：
- 认知层：认同"我"只是第一步，认知提高是前提但不是最终目的。
- 行动层：做出成果才是实在的，反对"无病呻吟"；区分"破坏性叛逆"与"建设性叛逆"。
- 影响层：真正的进化不是"证明自己"，而是"为世界带来光"；打破丛林法则，在系统缝隙里做出小选择。

### 6.3 AI 分析边界

AI 可以做：

- 分析玩家选择体现出的人格倾向。
- 根据玩家画像生成 NPC 的表达方式。
- 决定 NPC 释放线索的等级。
- 在预设结局中选择最符合玩家的结局。
- 生成个性化旁白。

AI 不可以做：

- 临时创造新的主线真相。
- 改写角色核心设定。
- 直接告诉玩家所有答案。
- 跳过关键剧情节点。
- 随机生成不受控结局。

## 7. AI 接口流程

```mermaid
sequenceDiagram
    participant P as 玩家
    participant C as Client 前端
    participant S as Server 后端
    participant L as LLM 大模型

    P->>C: 点击选项
    C->>C: 更新 GameState
    C->>S: POST /api/analyze-choice
    S->>S: 组装 analyzeChoicePrompt
    S->>L: 调用大模型
    L-->>S: 返回人格分析 JSON
    S-->>C: 返回 result
    C->>C: 存入 aiTraces
    C-->>P: 显示 AI 分析结果
```

## 8. 当前版本能做什么

当前框架已经支持：

- **融合探索与叙事**：Phaser 像素地图始终可见，对话以浮动叠层呈现。
- WASD 移动主角，E 键交互，step-on trigger 自动触发。
- 通过 GameBridge 事件总线在 Phaser 和 React 之间通信。
- 底部浮动 DialogOverlay：角色立绘（主角左/NPC右/旁白无）+ 紧凑选项。
- 点击"继续"推进剧情。
- 点击选项改变人格值。
- 本地存档与读取。
- 请求 mock AI 进行选择分析。
- 请求 mock AI 生成 NPC 对话。
- 请求 mock AI 进行结局裁决。
- 显示调试用玩家画像面板。
- 显示调试用 AI 输出面板。

## 9. 后续优先开发任务

```mermaid
flowchart TD
    A[阶段1: 跑通前端剧情流程] --> B[阶段2: 扩展 scenes.ts 到第1-8章关键节点]
    B --> C[阶段3: 加入3班逃生与规则压力系统]
    C --> D[阶段4: 接入真实大模型]
    D --> E[阶段5: 完善 NPC 角色卡与线索释放]
    E --> F[阶段6: 加入结局池与正式结局裁决]
    F --> G[阶段7: 美术、音效、转场、演示视频]
```

## 10. 给编程 AI 的注意事项

1. 不要把剧情写死在 React 组件里，应写在 `data/scenes.ts`。
2. 不要在前端放任何大模型 API Key。
3. 不要让 AI 直接改写主线事实，AI 只负责分析、表达、裁决。
4. 选项影响应优先修改 `traits`，不要直接跳到“好结局/坏结局”。
5. 新增功能时先保证 TypeScript 编译通过。
6. 每次新增 scene，都要确认 `nextSceneId` 指向存在的节点。
7. 比赛 Demo 目标是“短而完整”，不要一开始做开放世界。
